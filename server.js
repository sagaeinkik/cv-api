...
app.get('/api/cv', (req, res) => {
    ...
    db.query(
        `SELECT id, employer, title, description, TO_CHAR(start_date, 'YYYY-MM-DD') AS start_date, TO_CHAR(end_date, 'YYYY-MM-DD') AS end_date FROM cv ORDER BY start_date DESC;
    `,
        (err, results) => {
            if (err) {
                //Hantera error
